...
def mean_aggregator(node_features,neigh_features,out_dims,scope_name):
    ...
    with tf.compat.v1.variable_scope(scope_name,reuse=tf.compat.v1.AUTO_REUSE) as scope:
        ...
        combined = tf.reduce_mean(tf.compat.v1.layers.dense(to_feats,units=out_dims,activation=cfg.act),axis=1)
    return combined
...
def pooling_aggreagtor(node_features,neigh_features,out_dims,scope_name):
    ...
    with tf.compat.v1.variable_scope(scope_name,reuse=tf.compat.v1.AUTO_REUSE) as scope:
        if cfg.concat:
            ...
            to_feats = neigh_features
        combined = tf.reduce_max(tf.compat.v1.layers.dense(to_feats,units=out_dims,activation=cfg.act),axis=1)
    return combined
