...
from models.GraphSAGE_LCM import GraphSAGE_LC
...
label_input = np.array([np.array(range(num_classes)) for i in range(len(df_data))])

model_input = model_input + [label_input]
# train
model = GraphSAGE_LC(feature_dim=features.shape[1],
...
def lcm_loss(y_true,y_pred,alpha=2):
            pred_probs = y_pred[:,:num_classes]
            label_sim_dist = y_pred[:,num_classes:]
            simulated_y_true = K.softmax(label_sim_dist + alpha*tf.cast(y_true, tf.float32))
...
model.compile(adam_v2.Adam(0.0001), lcm_loss)
...
# predict
predictions = model.predict(model_input, batch_size = A.shape[0])[:, :num_classes]
